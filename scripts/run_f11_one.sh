...
# --- remove first char (F) from the configuration name
CONFIG=${CONFIG:1}
...
prepare_all_pathnames()
{
	...
	DATADIR=$ROOT"/evaluation/measured/figure$DIR_SUFFIX/$BENCHMARK"
        ...
        RUNDIR=$DATADIR/$(hostname)-config-$BENCHMARK-F$CONFIG-$(date +"%Y%m%d-%H%M%S")

	mkdir -p $RUNDIR
        if [ $? -ne 0 ]; then
                echo "Error creating output directory: $RUNDIR"
        fi
	OUTFILE=$RUNDIR/perflog-$BENCHMARK-$(hostname)-F$CONFIG.dat
}
...
{
	CURR_CONFIG=$1
	echo "In Launch interference: $1"
        LAST_CHAR=${CURR_CONFIG: -1}
        if [ $LAST_CHAR == "M" ]; then
            CURR_CONFIG=${CURR_CONFIG::-1}
        fi
	FIRST_CHAR=${CURR_CONFIG:0:1}
	# --- take care of THP configs
	if [ $FIRST_CHAR == "T" ]; then
		CURR_CONFIG=${CURR_CONFIG:1}
	fi
	if [ $CURR_CONFIG == "LPRDI" ] || [ $CURR_CONFIG == "RPILD" ] || [ $CURR_CONFIG == "RPIRDI" ]; then
		echo "Launching Interference: $CURR_CONFIG"
...
launch_benchmark_config()
{
	...
	echo "$BENCHMARK : F$CONFIG completed."
        echo ""
	killall bench_stream &>/dev/null
}
...
# ---- we need it on two sockets
gen_fragmenting_datasets()
{
	SIZE=$1
	OUTFILE=$MAIN"/datasets/$2"
	if [ ! -e $OUTFILE ]; then
		echo "Creating file for populating page cache. This will take a while..."
		dd if=/dev/urandom of=$OUTFILE bs=1048576 count=$1 > /dev/null 2>&1
		echo "File created successfully."
	else
		echo "File already present. Reusing existing file."
	fi
...
warm_page_cache()
{
	F1=$MAIN"/datasets/$FILE1"
	F2=$MAIN"/datasets/$FILE2"
	# --- we need to repeat this a few times
	echo "Launching clients to pupolate page cache."
	ITERATIONS=3
	for (( i=1; i<=$ITERATIONS; i++ )) do
		echo "Populating now...($i/$ITERATIONS)"
		numactl -c 0 -m 0 cat $F1 > /dev/null &
		PID1=$!
		numactl -c 1 -m 1 cat $F2 > /dev/null &
		PID2=$!
		wait $PID1
		wait $PID2
	done
	echo "Finished populating page-cache. Ready to run jobs."
}

get_system_config(){
	MEMSTR=$(free -m | grep Mem)
	MEM=$(echo ${MEMSTR##*: } | cut -d " " -f 1)
	NODESTR=$(numactl --hardware | grep available)
	NODE_MAX=$(echo ${NODESTR##*: } | cut -d " " -f 1)
	NODE_MAX=`expr $NODE_MAX`
	MEM_PER_NODE=$(( $MEM / $NODE_MAX))
	# --- increase somewhat
	MEM_PER_NODE=`expr $MEM_PER_NODE + 1024`
}
...
fragment_nodes()
{
	get_system_config
	# --- generate files
	gen_fragmenting_datasets $MEM_PER_NODE $FILE1
	gen_fragmenting_datasets $MEM_PER_NODE $FILE2

	# --- read files into memory
	warm_page_cache
}

# --- fragment the system first
fragment_nodes
